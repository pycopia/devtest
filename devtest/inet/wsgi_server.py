...
#
# New HTTP/2 WSGI server forked from example in hyper-h2 package.
# This is used as a lightweight local server for running the webui.
...
This server does not support HTTP/1.1: it is a HTTP/2-only WSGI server.
...
class H2Protocol(asyncio.Protocol):
    ...
    def window_opened(self, event):
        ...
        *one* data chunk in the sending queue at any time. The threading events
        ...
        if event.stream_id:
            ...
            for data in list(self._flow_controlled_data.values()):
    ...
    def request_received(self, event):
        ...
        # WSGI environ
        environ = _build_environ_dict(event.headers, s, peername)
...
def _build_environ_dict(headers, stream, peername):
    """
    ...
    environ = {
        ...
        'SERVER_SOFTWARE': 'devtest/1.0',
        'HTTPS': "on",
        'SSL_PROTOCOL': 'TLSv1.2',
        'REMOTE_ADDR': peername[0],
    ...
    environ['CONTENT_LENGTH'] = int(environ.get('HTTP_CONTENT_LENGTH', 0))
    return environ


def get_tls_context(name):
    tls_context = ssl.create_default_context(ssl.Purpose.CLIENT_AUTH)
    tls_context.options |= (
        ssl.OP_NO_TLSv1 | ssl.OP_NO_TLSv1_1 | ssl.OP_NO_COMPRESSION
    )
    tls_context.set_ciphers("ECDHE+AESGCM")
    tls_context.load_cert_chain(certfile="{}.crt.pem".format(name),
                                keyfile="{}.key".format(name))
    tls_context.set_alpn_protocols(["h2"])
    return tls_context


def run(app, address="localhost", port=8443):
    global APPLICATION

    APPLICATION = app
    ssl_context = get_tls_context(address)
    loop = asyncio.get_event_loop()
    coro = loop.create_server(H2Protocol, address, port, ssl=ssl_context)
...
if __name__ == '__main__':
    ...
    def test_app(env, start_response):
        if env['REQUEST_METHOD'] == 'POST':
            body = env['wsgi.input'].read(env["CONTENT_LENGTH"])
            print("body:", body)
            start_response("201", [])
            return []
