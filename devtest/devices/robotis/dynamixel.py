...
from systest import timers
from systest.io import serial
from systest.os import time
...
class DynamixelErrorSet:
  ...
  Attributes: instruction overload checksum range overheating angle_limit
  input_voltage
  """
    ...
    def instruction(self, onoff):
        (self._set_flag(DynamixelErrorType.Instruction)
         if onoff else self._clear_flag(DynamixelErrorType.Instruction))
    ...
    def overload(self, onoff):
        (self._set_flag(DynamixelErrorType.Overload)
         if onoff else self._clear_flag(DynamixelErrorType.Overload))
    ...
    def checksum(self, onoff):
        (self._set_flag(DynamixelErrorType.Checksum)
         if onoff else self._clear_flag(DynamixelErrorType.Checksum))
    ...
    def range(self, onoff):
        (self._set_flag(DynamixelErrorType.Range)
         if onoff else self._clear_flag(DynamixelErrorType.Range))
    ...
    def overheating(self, onoff):
        (self._set_flag(DynamixelErrorType.Overheating)
         if onoff else self._clear_flag(DynamixelErrorType.Overheating))
    ...
    def angle_limit(self, onoff):
        (self._set_flag(DynamixelErrorType.AngleLimit)
         if onoff else self._clear_flag(DynamixelErrorType.AngleLimit))
    ...
    def input_voltage(self, onoff):
        (self._set_flag(DynamixelErrorType.InputVoltage)
         if onoff else self._clear_flag(DynamixelErrorType.InputVoltage))
...
class Dynamixel:
    ...
    def read_value(self, address, length):
        self._protocol.send_message(self.motorid, Instruction.Read, bytes([address, length]))
...
class DynamixelAX18A(Dynamixel):
    ...
    def read_eeprom(self):
        ...
        self._min_voltage_limit = min_v_limit / 10.0
        self._max_voltage_limit = max_v_limit / 10.0
