...
class SampleData:
  ...
  """
    ...
    def heading(self):
        ...
        heads = [f"{n} ({u})" for n, u in zip(self.columns, self.units)]
        return " | ".join(heads)
...
def plot_samples(samples, events=None, column="main_current", width=8):
  ...
  Arguments:
      ...
      events: optional list of times (floats) within sample range to put
        markers.
  ...
  """
    ...
    assert isinstance(axs, axes.Axes)
    ...
    n_x = colvector.shape[0]
    stride = n_x // downsample_to
    downsampled = np.fromiter(
        (np.mean(a) for a in colvector[::stride]),
        colvector.dtype,
        count=downsample_to,
    )
    newtimes = (np.arange(0, downsample_to) * (time[1] - time[0]) * n_x / float(downsample_to) +
                time[0])  # noqa

    axs.plot(newtimes, downsampled)
    axs.set_title(f'{column} over time{" with events" if events else ""}.')
    if events is not None:
        events.sort()
        offset = downsampled.min()
        length = offset * 2
        evl = mpl_collections.EventCollection(
            events,
            color=[(1, 0, 0), (0, 1, 0)],
            linelength=length,
            lineoffset=offset,
    ...
    axs.set_xlabel(f"time ({xunit})")
    axs.set_ylabel(f"{column} ({yunit})")
    axs.grid(True)
    return fig
